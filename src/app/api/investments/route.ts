...
import { createInvestmentEntry, listInvestments } from "@/lib/module-data"
...
export async function GET(request: Request) {
  ...
  const entries = await listInvestments(session.user.id)
...
export async function POST(request: Request) {
  ...
  const id = await createInvestmentEntry({
